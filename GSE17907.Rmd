...
raw_grades <- pdata17907$`grade sbr:ch1`   
...
keep <- !is.na(pdata17907$grade)
pdata17907 <- pdata17907[keep, ]

expr17907 <- exprs(gse17907)[, keep]

stopifnot(all(rownames(pdata17907) == colnames(expr17907)))
table(pdata17907$grade)
```
...
if (!"sample" %in% colnames(pdata17907)) {
  pdata17907$sample <- rownames(pdata17907)
}

dge <- DGEList(counts = expr17907)

min_group_size <- min(table(pdata17907$grade))
...
expr17907_filtered <- expr17907[keepTheseGenes, ]


pdata17907_filtered <- pdata17907 %>% 
  filter(sample %in% colnames(expr17907_filtered))

expr17907_filtered <- expr17907_filtered[, pdata17907_filtered$sample]

stopifnot(all(colnames(expr17907_filtered) == pdata17907_filtered$sample))

# - expr17907_filtered：A matrix containing only highly expressed genes
# - pdata17907_filtered：pData that only contains these samples
...
samples_to_keep <- intersect(
  colnames(expr17907_filtered),
  rownames(pdata17907_filtered)
)

expr_mat <- expr17907_filtered[, samples_to_keep]
pdata_sub <- pdata17907_filtered[samples_to_keep, , drop = FALSE]
...
metadata_pf <- pdata17907_filtered
expr_pf     <- expr17907_filtered
