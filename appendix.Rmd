...
```
```{r}
cat("Dimensions of expr_train:\n")
print(dim(expr_train))  # genes x samples

cat("Dimensions of expr_test:\n")
print(dim(expr_test))

cat("Dimensions of metadata_train:\n")
print(dim(metadata_train))  # samples x variables

cat("Dimensions of metadata_test:\n")
print(dim(metadata_test))

cat("Sample names in expr_train vs metadata_train:\n")
print(all(colnames(expr_train) == metadata_train$sample))  # Should be TRUE
...
combine_cv_plot <- pivot_longer(summary_df, 
...
gse17907_eval_plot <- ggplot(eval_long_combined , aes(x = GeneSet, y = Value, fill = Model)) +
  geom_col(position = position_dodge(0.8)) +
  facet_wrap(~ Metric, scales = "free_y") +
  labs(
    title = "Test Set Evaluation: Extended Metrics",
...
gse15852_eval_plot <- ggplot(eval_long_combined , aes(x = GeneSet, y = Value, fill = Model)) +
  geom_col(position = position_dodge(0.8)) +
  facet_wrap(~ Metric, scales = "free_y") +
  labs(
    title = "Test Set Evaluation: Extended Metrics",
...
metrics_long <- all_metrics %>%
  select(Dataset, Model, GeneSet, BalancedAccuracy, MacroF1) %>%
