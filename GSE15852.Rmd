...
gse17907 <- process_eset(gse15852)
...
for (gene_set_name in names(top_gene_sets)) {
  ...
  x_train <- as.data.frame(t(expr_balanced[gene_set, , drop = FALSE]))
  x_test  <- as.data.frame(t(expr_test[gene_set, , drop = FALSE]))
  
  # Skip if insufficient samples for CV
  if (nrow(x_train) < cv_folds) {
    cat("Skipping", gene_set_name, "- insufficient samples for CV\n")
    next
  }
  ...
  # Model training
  svm_mod <- tryCatch(
    train(x = x_train, y = y_train, method = "svmLinear", trControl = trctrl, tuneGrid = data.frame(C = 1)),
    error = function(e) { warning(paste("SVM failed for", gene_set_name)); NULL }
  )
  ...
  # Save CV results
  if (!is.null(svm_mod)) acc_list[[length(acc_list) + 1]] <- data.frame(GeneSet = gene_set_name, Model = "SVM", Accuracy = svm_mod$resample$Accuracy)
  ...
  # Test set evaluation
  test_preds <- list(
    SVM = if (!is.null(svm_mod)) predict(svm_mod, x_test) else rep(NA, nrow(x_test)),
    KNN = predict(knn_mod, x_test),
    RF  = predict(rf_mod,  x_test)
  )
  
  for (model_name in names(test_preds)) {
    if (all(is.na(test_preds[[model_name]]))) next
...
ggplot(acc_df, aes(x = GeneSet, y = Accuracy, fill = Model)) +
  ...
  labs(title = "Model Accuracy (Linear selection)", x = "Top Gene Set", y = "Cross-validated Accuracy") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "top")

# Test Evaluation Bar Chart
eval_long <- pivot_longer(summary_df, cols = c("Accuracy", "MacroF1", "Kappa"), names_to = "Metric", values_to = "Value")

ggplot(eval_long, aes(x = GeneSet, y = Value, fill = Model)) +
  geom_col(position = position_dodge(0.8)) +
  facet_wrap(~ Metric, scales = "free_y") +
  labs(title = "Test Set Evaluation: Accuracy, Macro F1, Kappa", x = "Top Gene Set", y = "Metric") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "top")
