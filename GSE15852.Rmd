...
keep <- !is.na(pdata15852$grade)
pdata15852 <- pdata15852[keep, ]

expr15852 <- exprs(gse15852)[, keep]

stopifnot(all(rownames(pdata15852) == colnames(expr15852)))
table(pdata15852$grade)
```
...
if (!"sample" %in% colnames(pdata15852)) {
  pdata15852$sample <- rownames(pdata15852)
}

dge <- DGEList(counts = expr15852)

min_group_size <- min(table(pdata15852$grade))
...
expr15852_filtered <- expr15852[keepTheseGenes, ]


pdata15852_filtered <- pdata15852 %>% 
  filter(sample %in% colnames(expr15852_filtered))

expr15852_filtered <- expr15852_filtered[, pdata15852_filtered$sample]

stopifnot(all(colnames(expr15852_filtered) == pdata15852_filtered$sample))

# - expr15852_filtered：A matrix containing only highly expressed genes
# - pdata15852_filtered：pData that only contains these samples
...
samples_to_keep <- intersect(
  colnames(expr15852_filtered),
  rownames(pdata15852_filtered)
)

expr_mat <- expr15852_filtered[, samples_to_keep]
pdata_sub <- pdata15852_filtered[samples_to_keep, , drop = FALSE]
...
metadata_pf <- pdata15852_filtered
expr_pf     <- expr15852_filtered
