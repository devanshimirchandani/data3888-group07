...
keepTheseGenes <- rowSums(cpm(dge) > 1) >= 35
...
# ========== 5. The training/test sets are stratified by grade ==========
set.seed(123)
...
labels_test <- make.names(metadata_test$grade)
y_test <- labels_test
...
for (gene_set_name in names(top_gene_sets)) {
  ...
  for (cv in cv_folds) {
    ...
    svm_pred <- predict(svm_mod, x_test)
    knn_pred <- predict(knn_mod, x_test)
    ...
    acc_list[[length(acc_list) + 1]] <- data.frame(
      GeneSet = gene_set_name,
      CV = paste0("CV", cv),
      Model = "SVM",
      TestAccuracy = svm_acc
    )
    acc_list[[length(acc_list) + 1]] <- data.frame(
      GeneSet = gene_set_name,
      CV = paste0("CV", cv),
      Model = "KNN",
      TestAccuracy = knn_acc
    )
    acc_list[[length(acc_list) + 1]] <- data.frame(
      GeneSet = gene_set_name,
      CV = paste0("CV", cv),
...
ggplot(acc_df, aes(x = GeneSet, y = TestAccuracy, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_text(aes(label = round(TestAccuracy, 2)),
            position = position_dodge(width = 0.8),
            vjust = -0.5, size = 2.5) +
  facet_wrap(~ CV) +
  ylim(0, 1.05) +
  labs(
    title = "Test Accuracy under the linear selection of important genes",
    x = "Top Gene Set",
    y = "Test Accuracy"
  ) +
...
top_gene_sets_noliner <- list()
if (length(sig_genes_sorted) >= 30)  top_gene_sets_noliner$top30  <- sig_genes_sorted[1:30]
if (length(sig_genes_sorted) >= 50)  top_gene_sets_noliner$top50  <- sig_genes_sorted[1:50]
if (length(sig_genes_sorted) >= 80)  top_gene_sets_noliner$top80  <- sig_genes_sorted[1:80]
if (length(sig_genes_sorted) >= 110) top_gene_sets_noliner$top110 <- sig_genes_sorted[1:110]

cat("Available gene sets (ANOVA + t-test based):\n")
print(names(top_gene_sets_noliner))
...
for (gene_set_name in names(top_gene_sets_noliner)) {
  
  gene_set <- top_gene_sets_noliner[[gene_set_name]]
  x_train <- t(expr_balanced[gene_set, , drop = FALSE])
  x_test  <- t(expr_test[gene_set, , drop = FALSE])
...
ggplot(acc_df, aes(x = GeneSet, y = TestAccuracy, fill = Model)) +
  ...
  ylim(0, 1.05) +
  labs(
    title = "Test Accuracy under the Non-linear selection of important genes",
    x = "Top Gene Set",
    y = "Test Accuracy"
  ) +
  theme_minimal(base_size = 13) +
